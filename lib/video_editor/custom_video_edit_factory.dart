...
import 'custom_video_attribute.dart';

typedef MediaInformationSessionComple = int Function(
    MediaInformationSession session);

class CustomVideoEditFactory {
  late final String inputPath;

  late CustomVideoAttribute _videoAttribute;

  CustomVideoEditFactory({required this.inputPath}) {
    _videoAttribute = new CustomVideoAttribute();
  ...
  changeSpeed(double value) {
    _videoAttribute.speedUp = "-filter:v \"setpts=0.5*PTS\"";
  }

  cutByDoubleTime(double start, double end) {
    _videoAttribute.cutByTime = "-ss $start -to $end ";
  }
